...
class Figure():
    ...
    def plot(
        ...
        **kwargs
        ...
        is_iterable = self._anyIterables(x, y)
        create_daughter = self._shouldCreateDaughter(connect_data)
        ...
        if plot_type == "plot":
            ...
            if self.daughter is not None:
                ...
                for idx, point in enumerate(points):
                    ...
                    self._connectToDaughter(connect_data, line, idx)
    ...
    def _anyIterables(self, x, y):
        """ Check if x or y is an iterable """
        is_iterable = any(isinstance(data, (np.ndarray, list, tuple)) for data in (x, y))
        return is_iterable


    def _shouldCreateDaughter(self, connect_data: dict) -> bool:
    ...
    def _connectToDaughter(self, connect_data: dict, line: mpl.lines.Line2D, idx: int) -> bool:
